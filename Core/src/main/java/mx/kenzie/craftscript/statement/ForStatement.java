...
public record ForStatement(VariableAssignmentStatement assignment, Statement<?> then) implements Statement<Boolean> {
    ...
    public static Boolean execute(Context context, String var, Object set, Executable<?> function) {
        ...
        if (values instanceof Iterable<?> iterable)
            for (final Object value : iterable) trip(context, value, var, function);
        else if (values instanceof Iterator<?> iterator)
            while (iterator.hasNext()) trip(context, iterator.next(), var, function);
        else if (values instanceof Stream<?> stream)
            stream.forEach(object -> trip(context, object, var, function)); // hope this is ok
        else if (values instanceof Object[] objects)
            for (final Object value : objects) trip(context, value, var, function);
